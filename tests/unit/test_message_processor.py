...
async def test_ai_processor_can_process_when_flag_false():
    ...
    with patch('services.message_processor.ManagerMode') as mock_manager_mode:
        ...
        mock_manager_mode.get.return_value = mock_info_user

        result = await processor.can_process(message, account_id=1)
...
async def test_manager_processor_can_process():
    ...
    with patch('services.message_processor.ManagerMode') as mock_manager_mode:
        mock_manager_mode.get.return_value = None
...
def test_extract_kwork_user_id_from_mfrom():
    """Тест извлечения ID пользователя из mfrom"""
    message = {
        "mfrom": "account_name",
        "MSGFROM": 123,
        "MSGTO": 456
    ...
    assert user_id == 456  # Если это наш аккаунт, то берём MSGTO


def test_extract_kwork_user_id_from_msgfrom():
    """Тест извлечения ID пользователя из MSGFROM"""
    message = {
        "mfrom": "customer",
        "MSGFROM": 123,
        "MSGTO": 1
    ...
    user_id = AIMessageProcessor._extract_kwork_user_id(message, "account_name")
    assert user_id == 123  # Если это не наш аккаунт, то берём MSGFROM
